...
sample_enc_input_data_rev = next(iter(train_dataset))[0]

# 모델

sample_enc_input_data = tf.reverse(sample_enc_input_data_rev, axis=[1], name="input_reverse")
...
dec_output_rev = sample_decoder(enc_output)
# dec_output.shape

dec_output = tf.reverse(dec_output_rev, axis=[1], name="output_reverse")
